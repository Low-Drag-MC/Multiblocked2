...
public class Content {
    ...
    public void drawChance(GuiGraphics graphics, float x, float y, int width, int height) {
        ...
        String s = chance == 0 ? LocalizationUtils.format("mbd.gui.content.chance_0_short") : String.format("%.1f", chance * 100) + "%";
    ...
    public void drawTick(GuiGraphics graphics, float x, float y, int width, int height) {
        ...
        graphics.pose().translate(0, 0, 400);
        graphics.pose().scale(0.5f, 0.5f, 1);
        String s = LocalizationUtils.format("mbd.gui.content.tips.per_tick_short");
        int color = 0xFFFF00;
        Font fontRenderer = Minecraft.getInstance().font;
