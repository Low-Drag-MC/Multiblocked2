...
public class PNCHeatTraitFixtures implements TestFixtureProvider {
    public static final ResourceLocation MACHINE_ID = MBD2.id("test_pnc_heat_machine");

    @Override
    public void registerMachines(MBDRegistryEvent.Machine event) {
        var def = new PNCHeatExchangerTraitDefinition();
        def.setThermalCapacity(1000);
        def.setThermalResistance(1);
